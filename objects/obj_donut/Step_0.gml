if(place_meeting(x,y,obj_player)){
				...
				//messages all shown				
					if(victory) || (playerDead){
						...
						if(victory){
							...
							instance_destroy(obj_donut, true);
							game_end();
						}
					}else{	
	...
	if (!playerTurn) && (!showBattleText){
		...
		if (enemytimer >= timeTillEnemyAttacks){
			...
			ds_messages[|0] = "Destructive Donut Attacks";
			battleOption = 0;
			...
			if(roll == "Hits"){
				dmg = irandom(1) + 2;
				ds_messages[|1] = "Sick Tunes hits for "	+ string(dmg) + " damage!";
				
			}else{
				dmg = irandom(1) + 3;
				ds_messages[|1] = "Banger Beats hits for "	+ string(dmg) + " damage!";
			}
		}
	}
