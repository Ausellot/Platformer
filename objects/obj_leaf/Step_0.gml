if(place_meeting(x,y,obj_player)){
				...
				//messages all shown				
					if(victory) || (playerDead){
						...
						if(victory){
							...
							instance_destroy(obj_leaf, true);
						}
					}else{	
	...
	if (!playerTurn) && (!showBattleText){
		...
		if (enemytimer >= timeTillEnemyAttacks){
			...
			ds_messages[|0] = "Angry Leaf Attacks";
			battleOption = 0;
			...
			if(roll == "Hits"){
				dmg = irandom(1) + 1;
				ds_messages[|1] = "Leaf Lash hits for "	+ string(dmg) + " damage!";
				
			}else{
				dmg = irandom(1);
				ds_messages[|1] = "Roll hits for "	+ string(dmg) + " damage!";
			}
		}
	}
