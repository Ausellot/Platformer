if(place_meeting(x,y,obj_player)){
				...
				//messages all shown				
					if(victory) || (playerDead){
						...
						if(victory){
							...
							instance_destroy(obj_rock, true);
						}
					}else{	
	...
	if (!playerTurn) && (!showBattleText){
		...
		if (enemytimer >= timeTillEnemyAttacks){
			...
			ds_messages[|0] = "Brilliant Rock Attacks";
			battleOption = 0;
			...
			if(roll == "Hits"){
				dmg = irandom(1) + 2;
				ds_messages[|1] = "Boulder Bash hits for "	+ string(dmg) + " damage!";
				
			}else{
				dmg = irandom(1) + 1;
				ds_messages[|1] = "Diamond Drill hits for "	+ string(dmg) + " damage!";
			}
		}
	}
